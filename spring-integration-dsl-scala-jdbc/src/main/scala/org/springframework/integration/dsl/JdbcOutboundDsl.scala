 ...
 */
private[dsl] class JdbcOutboundGatewayConfig(name: String = "$file_out_" + UUID.randomUUID().toString.substring(0, 8),
  target: String,
  oneway: Boolean,
  fileNameGenerationFunction:_ => String) extends SimpleEndpoint(name, target) with OutboundAdapterEndpoint {
  ...
  override def build(document: Document = null,
    ...
    val element: Element =
      if (oneway) {
        ...
        if (fileNameGenerationFunction != null) {
          val fileNameGenerator = targetDefinitionFunction(fileNameGenerationFunction)
          val expressionParam =
